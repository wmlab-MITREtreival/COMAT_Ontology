...
sen_threshold=0.60
v_threshold = 0.6 # 0.95
o_threshold = 0.6 # 0.76
##############################################################################
...
model = SentenceTransformer('sentence-transformers/paraphrase-distilroberta-base-v2', device='cpu')
...
def word_embedding(sen):
    try:
        if not isinstance(sen, str) or len(sen.strip()) == 0:
            print("Invalid input for embedding:", sen)
            return None
        # Embed using CPU
        return model.encode(sen)
    except Exception as e:
        print(f"Error in word embedding generation for '{sen}': {e}")
        return None
...
def mapping(local_vo_pair_lst, v_in_verb, v_in_obj, match_vo_pair):
    for i,row in local_vo_pair_lst.iterrows():
        ...
        if local_match !=[]:
            ...
            # 開啟CSV檔案，以附加模式（append mode）寫入
            with open(csv_file_path, mode='a', newline='') as file:
                writer = csv.writer(file)
                if str(local_match[0]) == 'contain':
                    writer.writerow(local_match[0][1])
                    print("資料已成功存儲為CSV檔案。")
            # print("###############")
...
def attack_pattern_inference(vo_pair_embeddings, group=[], software=[], vo_pair=[], document="", srl_info=[], tactic_list=[]):
    ...
    #match_vo_pair = srl_multi_vo_pair_mapping(srl_info, tactic_list)
    ...
    print("############# inference #############")
    #print("match_vo_pair:",match_vo_pair)
